package de.Iclipse.BARO.Util.Command;


import com.google.common.base.Defaults;
import com.google.common.base.Joiner;
import de.Iclipse.BARO.Data;
import de.Iclipse.BARO.Util.TypeUtils;
import de.Iclipse.BARO.Util.executor.ThreadExecutor;
...
public class CommandProcessor<S> {
    ...
    public void process(S sender, FlagList.FilterResult flags, int wildcards, String[] args) {
        if (command.noConsole() && !masterCommand.getPlayerClass().isInstance(sender)) {
            ((CommandSender)sender).sendMessage(Data.prefix + "Keine Konsole");
            return;
        }

        if (command.requiresConsole() && masterCommand.getPlayerClass().isInstance(sender)) {
            ((CommandSender)sender).sendMessage(Data.prefix + "Nur Konsole");
            return;
        }
        ...
        if (!hasPermission) {
            ((Player) sender).sendMessage(Data.prefix + "§cKeine Berechtigung");
            return;
        }
